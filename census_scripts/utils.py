...
def format_request(base, endpoint, date, codes, state, county, key):
    ...
    if not 'subject' in endpoint:
        bg = f'for=block%20group:*'
        tract = f'in=tract:*'
        args = '&'.join(map(str, [codes, bg, state, county, tract, key]))
    else:
        bg=f'for=tract:*'
        args = '&'.join(map(str, [codes, bg, state, county, key]))
...
def small_batch_request(session, partition, func, arg, date, exceptions, state, county, debug=False):
    ...
    date = str(date)
    if debug:
        print(format_request(**func(arg, date, tmp[0], state, county)))
    try:
    ...
    for arg in endpoints[1:]:
        if type(res) != pd.DataFrame:
            ...
            if type(tmp) == pd.DataFrame:                
                ...
                res = res.merge(tmp, how = "inner", on=["state","tract","county"])
